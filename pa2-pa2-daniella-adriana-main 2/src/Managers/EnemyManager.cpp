...
// Define static members here
    ...
    bool EnemyManager::SpiralBossSeen = false; 
    ...
    int EnemyManager::shieldBar =0;
    bool EnemyManager::changeShip = false;
    bool EnemyManager::bossKilled = false;
...
void EnemyManager::manageCollisions(Player* player) {
  ...
  // Handle collisions between player bullets and enemies
    for (auto& enemy : enemyList) {
        ...
        for (auto& bullet : player->bullets) {
            if (!bullet.bulletIsOutOfBounds() && enemy->getHitBox()->isHit(bullet)) {
                player->health = min(player->health + 3.0, 100.0);
                 // Reward the player by healing them

                enemy->takeDamage(bullet.getDamage());            // Enemy will take damage from the bullet
                if (enemy->isDead()) {
                    player->shield = min(player->shield+1,10);
    ...
    for (auto& Boss : bossList) {
        ...
        for (auto& bullet : player->bullets) {
            ...
            if (!bullet.bulletIsOutOfBounds() && Boss->getHitBox()->isHit(bullet)) {
                ...
                if (Boss->isDead()) { 
                    player->shield = min(player->shield+10.0,10.0);                  //If the boss has died from a bullet
    ...
    for(auto& enemy:enemyList){
        if(player->hitBox.EnemyCollision(*enemy->getHitBox()))
        player->health = max(player->health - 0.2, 0.0);
    }

    for(auto& boss:bossList){
        if(player->hitBox.EnemyCollision(*boss->getHitBox()))
        player->health = max(player->health - 0.2, 0.0);
    }
...
void EnemyManager::decrementShieldBarTimer(){
    if(shieldBar > 0){
        --shieldBar;
    }
}
...
void EnemyManager::resetShieldBarTimer(int value){
    shieldBar = value;
...
void EnemyManager::spawnEnemy(Player* player){
    ...
    if (enemySpawnTimer >= spawnInterval) {
        ...
        if (!bossIsActive) { // Ensure no boss is currently active before spawning another
            ...
            else if(currentScore > 10000 && !SpiralBossSeen){
                initiateBossSpawn("Firulais");
                SpiralBossSeen = true;
            }
        }
        ...
        if (currentScore > 5000){
            enemyList.push_back(make_unique<ImperialInterceptor>(spawnLocation.x, spawnLocation.y));
        }
        else {
...
void EnemyManager::spawnBoss(const string& bossType) {
    ...
    else if(bossType == "Firulais"){
        SpiralBossSeen = true; 
        auto boss = make_unique<SpiralBoss>(ofGetWidth()/2, 20, "Firulais");
        bossList.push_back(move(boss));
    }
...
int EnemyManager::whichSpawnInterval(int playerScore) {
    ...
    if (!bossIsActive && SpiralBossSeen) return 5;
    ...
    if(playerScore >5000) return 120;
...
void EnemyManager::cleanUp() {
    ...
    ufoSeen = false;
    ortSeen = false;
    SpiralBossSeen = false;
    bossHasDied();
}

void EnemyManager::removeEnemiesOnlyCleanUp(){
    enemyList.clear();
}
void EnemyManager::removeEnemiesOnly(){
...
void EnemyManager::damageBosses(int damage){
    for (auto& boss : bossList) {
