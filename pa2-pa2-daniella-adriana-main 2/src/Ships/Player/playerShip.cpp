...
Player::Player(int Xposition, int Yposition){
    pos.x = Xposition;
    pos.y = Yposition;
    health = 100; //health level
    shield = 0;
    velocity.set(0, 0);
    this->shipSprite.load("ShipModels/shipModel2.png");
    this->shipSprite2.load("ShipModels/secondPlayer.png");
    ...
    lastShotTime = 0;
    shotCooldown = 0.5;  // Set the cooldown duration to 0.5 seconds (adjust as needed)
    isSprinting = false; 
}
...
void Player::draw2() {
        // Draw the ship sprite with the calculated rotation
            ...
            this->shipSprite2.draw(-20, -20, 45, 45);
...
void Player::update() {
    processPressedKeys();  // Process the pressed keys and calculate orientation change
     ...
     if (isSprinting) {
        maxSpeed = 10; // Double the maximum speed while sprinting
    } else {
        maxSpeed = 5; // Reset the maximum speed when not sprinting
    }
...
void Player::shoot2() { 
    ...
    // Check if enough time has passed since the last shot
        if (currentTime - lastShotTime >= shotCooldown) {
                ...
                Projectiles p = Projectiles(ofPoint(this->pos.x, this->pos.y), this->shipOrientation,100);
                p.setColors(ofColor::deepPink, ofColor::skyBlue);
                this->bullets.push_back(p);
...
void Player::addPressedKey(int key) {
    ...
    if (key == OF_KEY_SHIFT) {
        isSprinting = true; // Set the sprinting flag
    }
}
...
void Player::removePressedKey(int key) {
    ...
    if (key == OF_KEY_SHIFT) {
        isSprinting = false; // Reset the sprinting flag
    }
}
